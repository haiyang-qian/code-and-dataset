...
from confusion_metrics import ConfusionMatrix
...
from myvt import myvt_224_h8_edim64
from myvt_noclstok import myvt_224_h8_edim64_noclstok
from myvt_noclstok_noattn import myvt_224_h8_edim64_noclstok_noattn
...
def main():
    ...
    myvt_224_h8_edim64(pretrained=False)
    myvt_224_h8_edim64_noclstok(pretrained=False)
    myvt_224_h8_edim64_noclstok_noattn(pretrained=False)
    ...
    if not os.path.exists(train_dir):
        ...
        exit(1)
    dataset_train = CornDataset(train_dir)
    ...
    if not os.path.isdir(eval_dir):
        ...
        if not os.path.isdir(eval_dir):
            ...
            exit(1)
    dataset_eval = CornDataset(eval_dir)

    loader_eval_train = create_loader(
        dataset_train,
        input_size=data_config['input_size'],
    ...
    try:
        _logger.info("The train process is starting")
        for epoch in range(start_epoch, num_epochs):
            ...
            eval_train_metrics = train_validate(model, loader_eval_train, validate_loss_fn, args, amp_autocast=amp_autocast)
            train_metrics = train_metrics.copy()
            train_metrics.update(eval_train_metrics)
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix='',epoch=99):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    ...
    with torch.no_grad():
        ...
        confusion.summary()
        confusion_matrix_dict,confusion_matrix_pb_dict = confusion.get_epoch_matrix_dict()
        ap_write_json({str(epoch):confusion_matrix_dict},"./confusion_matrix.json")
...
def train_validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if args.local_rank == 0 and last_batch:
                log_name = 'train_eval' + log_suffix
                _logger.info(
                    '{0}: [{1:>4d}/{2}]  '
                    'Time: {batch_time.val:.3f} ({batch_time.avg:.3f})  '
                    'Loss: {loss.val:>7.4f} ({loss.avg:>6.4f})  '
                    'Acc@1: ({top1.avg:>7.4f})  '.format(
                        log_name, batch_idx, last_idx, batch_time=batch_time_m,
                        loss=losses_m, top1=top1_m))

    metrics = OrderedDict([('_eval_loss', losses_m.avg), ('_eval_top1', top1_m.avg)])

    return metrics


def ap_write_json(ap_dict,file_path):
    with open(file_path, 'r') as f:
        load_dict = json.load(f)
    f.close()
    with open(file_path,'w') as f:
        new_dict = dict(load_dict,**ap_dict)
