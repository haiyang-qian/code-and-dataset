...
default_cfgs = {
    ...
    'myvt_224_h8_edim64_noclstok':_cfg(
        mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5),
    ),
}
...
@register_model
def myvt_224_h8_edim64_noclstok(pretrained=False,**kwargs):
    embed_dim = 64
    embed_num_heads = 4
    kernel_size = 7
    stride = 4
    padding = 1
    model = MYVT_NOCLSTOK(embed_dim=embed_dim,embed_num_heads=embed_num_heads,kernel_size=kernel_size,
                 stride=stride,padding=padding,**kwargs)
    model.default_cfg = default_cfgs['myvt_224_h8_edim64_noclstok']
    if pretrained:
