...
default_cfgs = {
    ...
    'myvt_224_h8_edim64_noclstok_noattn':_cfg(
        mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5),
    ),
}
...
class MYVT_NOCLSTOK_NOATTN(nn.Module):
    def __init__(self,embed_dim,embed_num_heads,kernel_size,stride,padding,pooling_kernel_size=3,pooling_stride=2,pooling_padding=1,
        ...
        self.tokens_pos = nn.Parameter(torch.zeros(1, self.num_tokens, embed_dim))
        ...
        self.head = nn.Linear(self.num_tokens, num_classes) if num_classes > 0 else nn.Identity()
        self.congregate = Congregate(embed_dim)
    ...
    def forward_features(self, x):
        ...
        feature_tokens = self.congregate(feature_tokens)
        return feature_tokens
...
@register_model
def myvt_224_h8_edim64_noclstok_noattn(pretrained=False,**kwargs):
    embed_dim = 64
    embed_num_heads = 4
    kernel_size = 7
    stride = 4
    padding = 1
    model = MYVT_NOCLSTOK_NOATTN(embed_dim=embed_dim,embed_num_heads=embed_num_heads,kernel_size=kernel_size,
                 stride=stride,padding=padding,**kwargs)
    model.default_cfg = default_cfgs['myvt_224_h8_edim64_noclstok_noattn']
    if pretrained:
